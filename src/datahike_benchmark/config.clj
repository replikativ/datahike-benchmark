...
(def datahike-uris
  (for [sor [false true]
        ...
        base-config [;; missing tests and install instructions (also mutex error?): {:name "Redis (HHT)" :uri "datahike:redis:localhost:6379/"  :db :dh-redis-hht :index :datahike.index/hitchhiker-tree :store {:backend :redis :host "localhost" :port 6379  :path "performance"}}
                     {:backend "In-Memory (HHT)"
                      :uri     "datahike:mem://"
                      :store   {:backend :mem :path "performance-hht"}
                      :db      :dh-mem-hht
                      :index   :datahike.index/hitchhiker-tree}
                     {:backend "In-Memory (Set)"
                      :uri     "datahike:mem://"
                      :store   {:backend :mem :path "performance-ps"}
                      :db      :dh-mem-set
                     ...
                     {:backend "PostgreSQL (HHT)"
                      :uri     "datahike:pg://datahike:clojure@localhost:5440/"
                      :db      :dh-psql
                      :index   :datahike.index/hitchhiker-tree
                      :store   {:backend :pg :host "localhost" :port 5440 :username "datahike" :password "clojure" :path "performance"}}
                     {:backend "File-based (HHT)"
                      :uri     "datahike:file://"
                      :db      :dh-file
                      :index   :datahike.index/hitchhiker-tree
                      :store   {:backend :file :path "/tmp/file-performance"}}]]
    (let [new-path (str (get-in base-config [:store :path])
                        "-s" (if sor 1 0)
                        "-t" (if ti 1 0))
...
(def datomic-uris [{:lib            :datomic
                    :backend        "Datomic Mem"
                    :db             :dat-mem
                    :uri            "datomic:mem://performance"
                    :schema-on-read false
                    :temporal-index false}                  ;; not working on connect
                   {:lib            :datomic
                    :backend        "Datomic Free"
                    :db             :dat-free
...
(def hitchhiker-configs [{:lib            :hitchhiker
                          :backend        " Hitchhiker Tree (Datoms)"
                          :db             :hht-dat :uri "datoms"
                          :schema-on-read false
                          :temporal-index false}
                         {:lib            :hitchhiker
                          :backend        " Hitchhiker Tree (Values)"
                          :db             :hht-val :uri "values"
                          :schema-on-read false
                          :temporal-index false}])

(def db-configurations (vec (concat datahike-uris datomic-uris hitchhiker-configs)))
